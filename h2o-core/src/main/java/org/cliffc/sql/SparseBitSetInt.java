...
public class SparseBitSetInt {

  private static final int REPROBE_LIMIT=8; // Too many reprobes then force a table-resize

  // --- Bits to allow Unsafe access to arrays
  private static final int _Ibase  = UNSAFE.arrayBaseOffset(int[].class);
  private static final int _Iscale = UNSAFE.arrayIndexScale(int[].class);
  private static long rawIndex(final int[] ary, final int idx) {
    ...
    return _Ibase + ((long)idx * _Iscale);
  }
  ...
  // --- Sentinels -------------------------
  // I exclude 1 integer from the 2^32 possibilities, and test for it before
  // entering the main array.  The NO_KEY value must be zero, the initial
  // value set by Java before it hands me the array.
  private static final int NO_KEY = 0;
  private static final int TOMBSTONE = Integer.MAX_VALUE;
  ...
  public SparseBitSetInt( ) { this(MIN_SIZE,true); }
  ...
  public SparseBitSetInt( final int initial_sz ) { this(initial_sz,true); }
   ...
   *  speed improvement.  */
  public SparseBitSetInt( final boolean opt_for_space ) { this(1,opt_for_space); }
  ...
  public SparseBitSetInt( final int initial_sz, final boolean opt_for_space ) {
  ...
  private static final class CHM implements Serializable {
    ...
    private boolean CAS_key( int idx, int old, int key ) {
      return UNSAFE.compareAndSwapInt( _keys, rawIndex(_keys, idx), old, key );
    }
    ...
    // Simple constructor
    CHM( final SparseBitSetInt sbs, ConcurrentAutoTable size, final int logsize ) {
      _sbs = sbs;
      _size = size;
    ...
    // --- tst ----------------------------------------------------------
    private boolean tst( final int key ) {
    ...
    // --- set ---------------------------------------------------------
    private boolean set( final int key, boolean is_copy ) {
      final int hash = hash(key);
      final int len = _keys.length;
      ...
      while( true ) {               // Spin till we get a Key slot
        final int K = _keys[idx];   // Get current key
    ...
    boolean clr(final int e) { throw new RuntimeException("unimpl"); }
    ...
    private boolean copy_slot( int idx ) {
      ...
      while( (key=_keys[idx]) >= NO_KEY ) {
        int mkey = ~(key==NO_KEY ? TOMBSTONE : key); // marked key
