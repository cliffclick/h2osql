...
public class TSMB6 implements TSMB.TSMBI {
  ...
  @Override public long run() {
    ...
    Vec vid = TSMB.PERSON_HASINTEREST_TAG.vec("did");
    Vec vtg = TSMB.PERSON_HASINTEREST_TAG.vec("hasinterest_tag");
    long[] ptags = new BuildsTags3().doAll(vid,vtg)._tags;
    if( PRINT_TIMING ) { t=System.currentTimeMillis(); System.out.println("Restructure P1P2#"+ptags.length+" "+(t-t0)+" msec"); t0=t; }
  ...
  private static class BuildsTags3 extends MRTask<BuildsTags3> {
    transient long[] _tags;
    @Override protected void setupLocal() { _tags = new long[(int)_fr.numRows()]; }
    @Override public void map(Chunk cids, Chunk ctags) {
      for( int i=0; i<cids._len; i++ ) {
        int id = (int)cids.at8(i);
        AtomicUtils.LongArray.incr(_tags,id);
      }
    }
    @Override public void reduce( BuildsTags3 bld ) {
      if( _tags != bld._tags )
  ...
  private static class AllTags extends MRTask<AllTags> {
    long _cnt;
    final long[] _ptags;
    AllTags( long[] ptags ) { _ptags=ptags; }
    @Override public void map( Chunk pids ) {
      long cnt=0;
      for( int i=0; i<pids._len; i++ ) {
        int p2 = (int)pids.at8(i);
        SparseBitSetInt p13s = TSMB.P_KNOWS_P.get(p2); // Set of p2->p1 and p2->p3
        if( p13s != null ) {
          long sum=0;
          for( int p13 : p13s.rawKeySet() )
            if( p13!=0 )
              sum += _ptags[p13];
  ...
  private static class Err3 extends MRTask<Err3> {
    long _cnt;
    final long[] _ptags;
    Err3( long[]  ptags ) { _ptags=ptags; }
    @Override public void map( Chunk pids ) {
      long cnt=0;
      for( int i=0; i<pids._len; i++ ) {
        long p1 = pids.at8(i);
        SparseBitSetInt p12s = TSMB.P_KNOWS_P.get(p1); // Set of p1->p2
        if( p12s != null )
          cnt += _ptags[(int)p1]*p12s.fast_cardinality();
      }
      _cnt=cnt;
    }
  ...
  private static class Err4 extends MRTask<Err4> {
    long _cnt;
    final long[] _ptags;
    Err4( long[]  ptags ) { _ptags=ptags; }
    @Override public void map( Chunk pids ) {
      long cnt=0;
      for( int i=0; i<pids._len; i++ ) {
        long p1 = pids.at8(i);
        SparseBitSetInt p2s = TSMB.P_KNOWS_P.get(p1); // Set of p2->p13s
        if( p2s != null ) {
          long np1cnt = _ptags[(int)p1];
          for( int p2 : p2s.rawKeySet() ) // ForAll P2s, do P1
            if( p2 != 0 ) {
              SparseBitSetInt p3s = TSMB.P_KNOWS_P.get(p2); // Set of p2->p3s
  ...
  //  // err4 : ForAll P1's,
  //  // Correct for not(PKP(p1,p3))
  //  long err4 = new Err4X(ptags).doAll(TSMB.PERSON.vec("did"))._cnt;
  //  if( PRINT_TIMING ) { t=System.currentTimeMillis(); System.out.println("Err4# "+err4+" "+(t-t0)+" msec"); t0=t; }
  //  
  //  return cnt - err3 - err4;
  //}
  ...
  private static class Count2 extends MRTask<Count2> {
    ...
    @Override public void map( Chunk p1s, Chunk p2s ) {
      ...
      for( int i=0; i<p1s._len; i++ ) {
        ...
        SparseBitSetInt np1s = TSMB.P_KNOWS_P.get(p1);
        SparseBitSetInt np2s = TSMB.P_KNOWS_P.get(p2);
        cnt += _check_knows(p1,np1s,np2s) + _check_knows(p2,np2s,np1s);
      }
      _cnt=cnt;
    }
    // Check P1->P2->P3, P1!=P3, P1-!->P3 & count P3 tags.
    private long _check_knows(long p1, SparseBitSetInt p1s, SparseBitSetInt p3s) {
      long cnt=0;
      for( int p3 : p3s.rawKeySet() )
        if( p3!=0 && p1!=p3 &&
            (p1s==null || !p1s.tst(p3)) )
  ...
  public long run1() {
    ...
    Vec vid = TSMB.PERSON_HASINTEREST_TAG.vec("did");
    Vec vtg = TSMB.PERSON_HASINTEREST_TAG.vec("hasinterest_tag");
    NonBlockingHashMapLong<SparseBitSetInt> ptags = new BuildsTags1().doAll(vid,vtg)._tags;
    if( PRINT_TIMING ) { t=System.currentTimeMillis(); System.out.println("Restructure P1P2#"+ptags.size()+" "+(t-t0)+" msec"); t0=t; }

    // ForAll P1s...
    Vec p1s = TSMB.PERSON_KNOWS_PERSON.vec("dp1");
    Vec p2s = TSMB.PERSON_KNOWS_PERSON.vec("dp2");
    long cnt = new Count1(ptags).doAll(p1s,p2s)._cnt;
    if( PRINT_TIMING ) { t=System.currentTimeMillis(); System.out.println("ForAll P1,P2,P3 "+(t-t0)+" msec"); t0=t; }
  ...
  private static class BuildsTags1 extends MRTask<BuildsTags1> {
    transient NonBlockingHashMapLong<SparseBitSetInt> _tags;
    @Override protected void setupLocal() { _tags = new NonBlockingHashMapLong<SparseBitSetInt>((int)(_fr.numRows()>>3)); }
    @Override public void map(Chunk cids, Chunk ctags) {
      for( int i=0; i<cids._len; i++ )
  ...
  private static class Count1 extends MRTask<Count1> {
    long _cnt;
    final NonBlockingHashMapLong<SparseBitSetInt> _ptags;
    Count1( NonBlockingHashMapLong<SparseBitSetInt> ptags ) { _ptags=ptags; }
    @Override public void map( Chunk p1s, Chunk p2s ) {
      long cnt=0;
      for( int i=0; i<p1s._len; i++ ) {
